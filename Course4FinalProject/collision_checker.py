    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                circle_locations[:, 0] = path[j].x_points + self._circle_offsets * cos(path[j].t_points)
                circle_locations[:, 1] = path[j].x_points + self._circle_offsets * sin(path[j].t_points) 
                # --------------------------------------------------------------
            ...
            collision_check_array[j] = collision_free
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                score = 1.0 / scipy.spatial.distance.cdist(paths[i], 
                                                     goal_state, 'euclidean')
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            score += self._weight * 1.0 /scipy.spatial.distance.cdist(paths[i], 
                                                                                paths[j], 'euclidean')
                            # --------------------------------------------------
